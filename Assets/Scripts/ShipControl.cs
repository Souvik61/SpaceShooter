...
{
    protected static string s_ObjectPoolTag = "ObjectPool";

    protected NetworkObjectPool m_ObjectPool;
    ...
    //Three trans forms to spawn bullet
    [SerializeField]
    Transform m_bulletSpawnTr1;
    [SerializeField]
    Transform m_bulletSpawnTr2;
    [SerializeField]
    Transform m_bulletSpawnTr3;

    [SerializeField]
    protected ParticleSystem m_Thrust;
    [SerializeField]
    protected ParticleSystem m_Thrust1;

    [SerializeField]
    protected SpriteRenderer m_ShipGlow;

    [SerializeField]
    protected Color m_ShipGlowDefaultColor;
    ...
    protected ProgressBar m_EnergyBar;

    protected VisualElement m_PlayerUIWrapper;

    protected TextElement m_PlayerName;

    protected Camera m_MainCamera;

    public ParticleSystem.MainModule m_ThrustMain;
    public ParticleSystem.MainModule m_ThrustMain1;
    ...
    public float m_OldMoveForce = 0;

    public float m_OldSpin = 0;
    ...
    void Fire(Vector3 direction, Transform startPt)
    {
        fireSound.Play();
        ...
        var bulletGo = m_ObjectPool.GetNetworkObject(BulletPrefab).gameObject;
        bulletGo.transform.position = startPt.position;
    ...
    protected void HandleFrictionGraphics()
    {
    ...
    public void HandleBuffColors()
    {
        //Handle Thrusters
        m_ThrustMain.startColor = m_IsBuffed ? LatestShipColor.Value : m_ShipGlowDefaultColor;
        m_ThrustMain1.startColor = m_IsBuffed ? LatestShipColor.Value : m_ShipGlowDefaultColor;
    ...
    public void NewFireServerRpc()
    {
        if (Energy.Value >= 10)
        {
            ...
            if (TripleShotTimer.Value > NetworkManager.ServerTime.TimeAsFloat)
            {
                Fire(right, m_bulletSpawnTr1);
                Fire(right, m_bulletSpawnTr2);
                Fire(right, m_bulletSpawnTr3);
            }
            else if (DoubleShotTimer.Value > NetworkManager.ServerTime.TimeAsFloat)
            {
                Fire(right, m_bulletSpawnTr2);
